...
def play(word):
    ...
    while not guessed and tries > 0:
        word_or_letter = input('Введи букву или слово: ').upper()

        if not word_or_letter.isalpha():
            print('Введи БУКВУ или СЛОВО!!!')
        elif len(word_or_letter) > 1 and len(word_or_letter) != len(word):
            print(f'АЛЛО!!! ТЫ ЧТО НЕ ВИДИШЬ ИЗ СКОЛЬКИХ БУКВ СОСТОИТ СЛОВО? ({len(word)} букв)')
        elif word_or_letter in guessed_letters or word_or_letter in guessed_words:
            print('Уже было!!!')
        else:
            if len(word_or_letter) == len(word):
                if word_or_letter == word:
                    guessed = True
                    word_completion = word
                else:
                    print('Не угадал, дальше продолжай')
                    guessed_words.append(word_or_letter)
                    tries -= 1
            elif len(word_or_letter) == 1:
                if word_or_letter in word:
                    guessed_letters.append(word_or_letter)
                    word_as_list = list(word_completion)
                    indices = [i for i, letter in enumerate(word) if letter == word_or_letter]
                    for index in indices:
                        word_as_list[index] = word_or_letter
    ...
    if guessed:
        ...
        print(f'К сожалению, ты не угадал слово. Оно было: {word}')
